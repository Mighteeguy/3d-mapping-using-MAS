...
class MovementHandler:
    ...
    def __init__(self, core, datastore):
        ...
        self.velocity_publisher = rospy.Publisher('/'+uav_prefix+'/cmd_vel', Twist, queue_size=10)
    ...
    def request_velocity(self, lin_x=None, lin_y=None, lin_z=None, ang_x=None, ang_y=None, ang_z=None):
        ...
        if lin_x is not None:
            self.current_vel_msg.linear.x = lin_x * 1
        if lin_y is not None:
            self.current_vel_msg.linear.y = lin_y * 1
        if lin_z is not None:
            self.current_vel_msg.linear.z = lin_z * 1
