...
class DataStore:
    ...
    def __init__(self):
        ...
        rospy.Subscriber('/'+uav_prefix+'/sonar_height', Range, self.sonar_height_callback)
        rospy.Subscriber('/map_2d', OccupancyGrid, self.map_callback)
        rospy.Subscriber('/'+uav_prefix+'/move_base/global_costmap/costmap', OccupancyGrid, self.global_cost_map_callback)
        rospy.Subscriber('/'+uav_prefix+'/move_base/global_costmap/costmap_updates', OccupancyGridUpdate,
    ...
    def update_last_pose(self):
        if len(sys.argv) < 2:
            ...
            uav_prefix = sys.argv[1]
        try:
            tfmsg = self.tf_buffer.lookup_transform('map', uav_prefix+'/base_link', rospy.Time())
